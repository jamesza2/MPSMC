...
int main(int argc, char *argv[]){
	...
	ThermalWalkers tw(sites, itev, tau, max_bd, 0, num_walkers, num_max_walkers);
	tw.set_kept_singular_values(kept_singular_values);
	tw.verbose = verbose;

	ThermalWalkers tw2(sites, itev, tau, max_bd, truncated_bd, num_walkers, num_max_walkers);
	tw2.set_kept_singular_values(0);
	tw2.verbose = verbose;
	
	if(trial_wavefunction_file_name != ""){
		tw.set_trial_wavefunction(trial);
	}
	itensor::MPS trial_copy(tw.trial_wavefunction);
	tw2.set_trial_wavefunction(trial_copy);
	tw2.walkers[0] = itensor::MPS(trial_copy);
	tw2.weights[0] = std::sqrt(itensor::inner(trial_copy, trial_copy));
	...
	vector<double> average_energies_2;
	vector<vector<double>> walker_energies_2;
	vector<vector<double>> walker_weights_2;
	vector<vector<double>> walker_overlaps_2;
	vector<int> num_current_walkers_2;
	vector<vector<int>> bds_2;
	vector<double> trial_energies_2;
	vector<double> entanglement_entropies_2;
	for(int iteration = 0; iteration < num_iterations; iteration++){
		...
		trial_energies_2.push_back(tw2.trial_energy);
		if(iteration > 0){
			tw2.iterate_single();
		}
		double energy_2 = tw2.expectation_value(H);
		...
		average_energies_2.push_back(energy_2);
		num_current_walkers_2.push_back(tw2.weights.size());
		entanglement_entropies_2.push_back(tw2.average_entanglement_entropy(num_sites/2));
		bds_2.push_back(tw2.get_bds());
	...
	out_file << "\n#AVERAGE_ENERGIES_2:\n";
	for(double energy : average_energies_2){
		out_file << energy << " ";
	}
	out_file << "\n#AVERAGE_ENTROPIES:\n";
	...
	out_file << "\n#AVERAGE_ENTROPIES_2:\n";
	for(double entropy : entanglement_entropies_2){
		out_file << entropy << " ";
	}
	out_file << "\n#NUM_WALKERS:\n";
	...
	out_file << "\n#NUM_WALKERS_2:\n";
	for(double nw : num_current_walkers_2){
		out_file << nw << " ";
	}
	out_file << "\n#TRIAL_ENERGIES:\n";
	...
	out_file << "\n#TRIAL_ENERGIES_2:\n";
	for(double energy : trial_energies_2){
		out_file << energy << " ";
	}
	out_file << "\n#WALKER_ENERGIES:";
	...
	out_file << "\n#WALKER_ENERGIES_2:";
	for(vector<double> walker_energy_vector : walker_energies_2){
		out_file << "\n";
		for(double walker_energy : walker_energy_vector){
			out_file << walker_energy << " ";
		}
	...
	out_file << "\n#WALKER_OVERLAPS_2:";
	for(vector<double> walker_overlap_vector : walker_overlaps_2){
		out_file << "\n";
		for(double walker_overlap : walker_overlap_vector){
			out_file << walker_overlap << " ";
		}
	...
	out_file << "\n#WALKER_WEIGHTS_2:";
	for(vector<double> walker_weight_vector : walker_weights_2){
		out_file << "\n";
		for(double walker_weight : walker_weight_vector){
			out_file << walker_weight << " ";
		}
	...
	out_file << "\n#BOND_DIMENSIONS_2:";
	for(vector<int> bond_dimension_vector : bds_2){
		out_file << "\n";
		for(int bd : bond_dimension_vector){
			out_file << bd << " ";
		}
